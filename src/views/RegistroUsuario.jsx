...
export const RegistroUsuario = () => {
  ...
  const registrarUsuario = async () => {
    const { tipoUsuario, nombre, dni, correo, password } = dataRegister;

    if (!tipoUsuario || !nombre || !dni || !correo || !password) {
    ...
    const response = await services({
      method: "POST",
      service: "http://localhost:5000/register",
      body: {
        nombre: dataRegister.nombre,
        correo: dataRegister.correo,
        contraseña: dataRegister.password,
        tipo_usuario: dataRegister.tipoUsuario,
        dni: dataRegister.dni,
      },
    });

    if (response.status === 201) {
      Alert("success", response.data.mensaje || "Registro exitoso");
      setDataRegister({
        tipoUsuario: "",
        nombre: "",
        dni: "",
      ...
      Alert(
        "error",
        response.data.mensaje || "Ocurrió un error al registrar el usuario"
      );
    }
    hideLoader();
  };
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="border-2 p-10 border-black mx-4 lg:w-100 flex flex-col justify-center bg-white shadow-md">
            ...
            <select
              ...
              value={dataRegister.tipoUsuario}
              id="tipo-usuario"
          ...
          <NavLink to="/" className="text-[#8c3836] underline">
            Volver
          </NavLink>
        </div>
      </div>
